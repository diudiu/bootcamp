...
class Activity(models.Model):
    ...
    def monthly_activity(user):
        """Static method to retrieve monthly statistical information about
        the user activity.
        @requires: user - Instance from the User Django model.
        @returns:  Two JSON arrays, the first one is dates which contains all the dates
                   with activity records, and the second one is datapoints containing
                   the sum of all the activity than had place in every single month
        Both arrays keep the same order, sor there is no need to order them.
        """
    ...
    def daily_activity(user):
        """
        Static method to retrive daily statiscial information about the user activiry.
        :param user - Instance from the User Django model.
        :return: Two JSON arrays, the first one is dates which contains all
